...
export class ASTParserFacade {
   ...
   * @returns {Program}
   */
  private static parseType (sourceCode: string, inputConfig: acorn.Options, sourceType: acorn.Options['sourceType']): ESTree.Program {
    ...
    const program: acorn.Node & ESTree.Program = <acorn.Node & ESTree.Program>acorn.Parser.extend(importAttributesOrAssertions).parse(sourceCode, config);
